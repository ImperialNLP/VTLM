...
DUMP_PATH=/data/ozan/experiments/mmvc/mmvc_code/multi30k_mmt_ftune_v4_periodic_noinit_dec
...
CKPT_NAME=`basename $CKPT1 | tr -- '-.' '_'`
CKPT_ID=$(basename `dirname $CKPT1`)
CKPT_NAME="${CKPT_ID}_${CKPT_NAME}"

LOG="`dirname $CKPT1`/train.log"
# Fetch previous args
...
python $TRAIN --beam_size 1 --exp_name ${NAME} --dump_path ${DUMP_PATH} \
  --reload_model "${CKPT1},${CKPT2}" --data_path ${DATA_PATH} --encoder_only false \
